...
(function ($) {
	...
	$.widget("wijmo.wijtabs", {
		...
		_blindPanel: function (panel, mode) {
			var o = this.options,
				content = panel.parent('.wijmo-wijtabs-content'),
				props = ['position', 'top', 'left', 'width'],
				blindOption = mode === 'show' ? o.showOption : o.hideOption,
				wrapper, a, listWidth,
				self = this;
			if (!content.length) {
				return;
			...
			if (mode === 'show') {
				// Shift
				wrapper.css($.extend({ width: 0 },
					blindOption.fade ? { opacity: 0} : {}));
			}

			// Animation
			a = $.extend({
				width: mode === 'show' ? this.element.data('panel.outerWidth') : 0
			}, blindOption.fade ? { opacity: mode === 'show' ? 1 : 0} : {});

			listWidth = this.list.outerWidth(true);
			// Animate
			wrapper.animate(a, {
		...
		_tabify: function (init) {
			...
			var self = this, o = this.options,
				...
				tabsAlign, panelCorner, content, i, li,
				addState, removeState, showTab, hideTab;
			this.anchors.each(function (i, a) {
				...
				// Thus normalize href attribute...
					hrefBase = href.split('#')[0], baseEl, id, $panel;
				...
				// prevent loading the page itself if href is just "#"
					...
					if (!$panel.length) {
						$panel = $(o.panelTemplate ||
							self._defaults.panelTemplate).attr('id', id)
			...
			if (init) {
				...
				this.element.addClass('ui-tabs wijmo-wijtabs' +
					' ui-tabs-' + tabsAlign + 
					' ui-widget ui-widget-content ui-corner-all ui-helper-clearfix');
				...
				case 'left':
					content = $('<div/>').addClass('wijmo-wijtabs-content')
			...
			hideTab = ((o.hideOption.blind || o.hideOption.fade) && 
				o.hideOption.duration > 0) ?
		...
		add: function (url, label, index) {
			...
			var self = this, o = this.options,
				$li = $((o.tabTemplate || self._defaults.tabTemplate)
					.replace(/#\{href\}/g, url).replace(/#\{label\}/g, label)),
				id = !url.indexOf('#') ? url.replace('#', '') : 
					this._tabId($('a', $li)[0]),
				tabsAlign = this._getAlignment(),
				panelCorner = this._getAlignment(false),
				$panel = $('#' + id),
			...
			$panel.addClass('ui-tabs-panel ui-widget-content ui-corner-' + 
				panelCorner + ' ui-tabs-hide').attr('aria-hidden', true);
			...
			if (index >= this.lis.length) {
				...
				if (this.panels.length > 0) {
					...
					$content = this.element.find('.wijmo-wijtabs-content');
					if ($content.length === 0) {
		...
		load: function (index) {
			...
			if (o.spinner || self._defaults.spinner) {
				span.data('label.tabs', span.html())
					.html(o.spinner || self._defaults.spinner);
			}
